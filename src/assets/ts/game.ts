...
function endTurnBeforeSecondVote(gameData: GameData, room: string) {
  const player = gameData.players.filter(player => player.isOnVote)
...
export function startVoting(room: string, gameData: GameData) {
  ...
  if (gameData.votingFor !== 0) {
    ...
    const maxVoteCountVotees = votees.reduce(
      (prev, current) => {
        return prev[0].voteCount < current.voteCount ? [current] : prev[0].voteCount === current.voteCount ? [...prev, current] : prev
      }, [votees.shift()!]
    )
    if (maxVoteCountVotees.length >= 2) {
      ...
      if (gameData.gameState === GameState.Vote) {
        ...
        gameData.currentTurn = maxVoteCountVotees.sort((a, b) => a.position - b.position)[0].position;
  ...
  let WinnerCountdown = setInterval(function () {
    ...
    if (counter === 0) {
      ...
      if (gameData.players.length/2 < gameData.votesForHangCount){
        gameData.players.filter(player => player.isOnVote).forEach(player => player.alive = false);
        //TODO last words
      }
        gameData.clear();
        gameData.gameState = GameState.MafiaShoot;
